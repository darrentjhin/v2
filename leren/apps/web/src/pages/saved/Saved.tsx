  ...
  return (
    <div className="flex gap-4 h-[calc(100vh-5rem)]">
      <div className="w-72 shrink-0 flex flex-col gap-3">
        <h1 className="text-xl font-bold">Saved Explanations</h1>
        <input className="input" placeholder="Search…" value={q} onChange={e => { setQ(e.target.value); load(e.target.value); }} />
        <div className="flex-1 overflow-y-auto space-y-2">
          {items.length === 0 && <p className="text-slate-500 text-sm mt-4 text-center">No saved explanations yet.</p>}
          {items.map(item => (
            <button key={item.id} onClick={() => setSelected(item)} className={`w-full text-left p-3 rounded-lg border transition-colors ${selected?.id === item.id ? 'border-blue-500 bg-blue-600/10' : 'border-slate-700 bg-slate-800 hover:bg-slate-700'}`}>
      ...
      <div className="flex-1 card overflow-y-auto">
        {selected ? (
          <>
            ...
            <p className="text-slate-300 whitespace-pre-wrap text-sm leading-relaxed">{selected.content}</p>
          </>
        ) : (
          <div className="flex items-center justify-center h-full text-slate-500">Select an explanation to view it</div>
        )}
      </div>
    </div>
  );
