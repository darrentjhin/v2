...
export function Sidebar() {
  ...
  return (
    <aside className="flex flex-col w-60 min-h-screen border-r border-slate-800/60 px-3 py-5 shrink-0" style={{ background: 'rgba(0,10,25,0.7)', backdropFilter: 'blur(12px)' }}>
      <div className="relative flex justify-center items-center w-full mb-8 min-h-[126px]">
        <Link to="/" className="flex justify-center items-center w-full">
          <img src="/logo.png" alt="Leren" className="leren-logo" />
        </Link>
        {user?.plan === 'PRO' && (
          <span className="absolute right-0 top-1/2 -translate-y-1/2 badge bg-blue-600/30 text-blue-300">PRO</span>
        )}
      </div>
