...
interface SessionContextValue {
  ...
  startSession:   (subject?: SubjectCtx, withScreenShare?: boolean) => Promise<void>;
...
export function SessionProvider({ children }: { children: ReactNode }) {
  ...
  // ── screen capture ───────────────────────────────────────────────────
  const captureScreenFrame = useCallback((): Promise<Blob | null> => {
  ...
  // ── Start session ─────────────────────────────────────────────────────
  const startSession = useCallback(async (subject?: SubjectCtx, withScreenShare = true) => {
    setError('');
  ...
  return (
    <SessionContext.Provider value={{ sessionId, activeSubject, turns, status, error, screenActive, startSession, endSession }}>
      {children}
